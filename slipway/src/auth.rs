...
impl AppState {
    ...
    pub fn from_configured_db(db: toasty::Db, cookie_key: Arc<Key>, secure_cookies: bool) -> Self {
        let mut state = Self::from_db_with_cookie_key(db, cookie_key);
...
pub fn secure_cookies(cx: &Cx) -> impl Cookies {
    cookies(cx)
        ...
        .default_secure(app_context::<AppState>(cx).secure_cookies)
