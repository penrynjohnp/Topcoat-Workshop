...
> [!WARNING]
> Topcoat is pre-1.0. The API surface changes between releases, and an AI model may confidently combine examples from different versions. Read the documentation for the pinned release tag, never `main`: for this workshop that means [Topcoat v0.8.1 on docs.rs](https://docs.rs/topcoat/0.8.1/) and the [v0.8.1 source guides](https://github.com/tokio-rs/topcoat/tree/v0.8.1).
...
> You are working in this Topcoat Workshop repository. Follow `AGENTS.md`. The project is pinned to Topcoat 0.8.1 and Toasty 0.10.0. Read v0.8.1 documentation, never `main`. Topcoat is server-rendered: do not move page data into browser storage, a client bundle, or a separate API layer. Explain the smallest change, show why it matches the compiler error, and give the existing validation command.
...
> Explain this Topcoat 0.8.1 `view!` compiler error as if I am a Rust web developer who knows HTML but not Topcoat's runtime-expression vocabulary. First identify whether the error comes from ordinary server-side Rust or from `$(...)` cross-compilation. Then point to the exact unsupported syntax, explain why the expression must work in both Rust and JavaScript, and propose two v0.8.1-compatible rewrites. Do not invent an API. Check the release-tagged `expr!` guide. End with the smallest code change and the command I should run to verify it.
...
> Classify this diagnostic as (a) Rust ownership/type checking, (b) `view!` markup syntax, or (c) Topcoat runtime-expression code generation. Quote the relevant v0.8.1 guide section and explain what evidence in the error supports your classification. Do not propose a workaround until the classification is clear.
...
> examples will often produce the second form. (v0.8.1 `runtime.md`, §Shards and §Reading signals on
> the server.)
...
- The page owns the signal, so the state survives: 0.8.1 morphs the shard's new markup into the existing DOM rather than replacing it, which keeps focus and partly-typed input alive. Reorderable list items need a stable `id` for the morph to track them.
...
> This suggestion fails against Topcoat 0.8.1. Here is the exact compiler output: `<paste compiler output>`. Here is the source at `<path>`. Compare the suggestion with the release-tagged v0.8.1 guide and identify the first incorrect assumption. Do not switch to a newer API or change the dependency version. Return either a minimal patch or state that the requested behaviour is unavailable in v0.8.1.
...
|---|---|---|
| Version drift | A `main`-branch macro, extractor, or helper exists in 0.8.1 | Read the v0.8.1 docs.rs page and release-tagged guide |
| Pre-0.8 signal syntax | A signal is declared as a statement inside `view!`, as `signal query = String::new();` | Compile it. 0.8.1 fails with `error: expected view node`. Signals are created with `signal(cx, String::new)` in the component body, and the component takes `cx: &Cx` (v0.8.1 `runtime.md`, §Signals) |
...
| Runtime-expression overreach | Any Rust expression works inside `$(...)` | Read the v0.8.1 `expr!` shared vocabulary and compile the lab |
...
| Streaming confusion | `live!` is a later request like a shard, or headers can change after streaming starts | Compare the v0.8.1 `live!` guide with the reactivity lab |
...
> Review this proposed patch against `AGENTS.md`, `PLAN.md`, `COMPATIBILITY.md`, and the v0.8.1 Topcoat guide. Check server-rendering boundaries, endpoint authorization, supported runtime expressions, source includes, formatting, and test coverage. Report only concrete mismatches with file and line references. Do not redesign working code.
