...
async fn main() {
    ...
    let url = std::env::var("SLIPWAY_DATABASE_URL")
        .unwrap_or_else(|_| lab11_starter::database::DEFAULT_DATABASE_URL.to_owned());
    let state = lab11_starter::auth::AppState::connect(&url)
        .await
        .expect("connect to the Slipway database");
    topcoat::start(lab11_starter::app::router(state, Some(assets)))
        .await
        .unwrap();
}
