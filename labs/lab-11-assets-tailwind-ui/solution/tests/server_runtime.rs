use lab11_solution::{auth::AppState, models::WorkOrder};
...
async fn vessel_shard_returns_only_the_filtered_fragment() {
    let router = lab11_solution::app::router(AppState::test().await.unwrap(), None);
    let page = router
        .handle(
            Request::builder()
                .method(Method::GET)
...
async fn procedure_persists_completion_and_history_streams_errors_in_place() {
    ...
    let router = lab11_solution::app::router(state.clone(), None);
