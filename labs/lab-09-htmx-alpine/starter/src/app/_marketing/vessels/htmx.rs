...
async fn vessels_htmx(cx: &Cx) -> Result<impl View> {
    ...
    Ok(view! {
        ...
        <form method="get" action="/vessels/htmx">
            ...
            // TODO(lab-09): make this input drive htmx. Add `hx-get` pointing at
            // `/vessels/htmx/results`, `hx-trigger` with a 250 ms debounce
            // (`input changed delay:250ms, search`), `hx-target` aimed at
            // `#vessel-results-htmx`, and `hx-indicator` aimed at
            // `#vessel-spinner`.
            <input id="vessel-query-htmx" type="search" name="q" value=(&query)>
