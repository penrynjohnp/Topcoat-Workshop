...
/// The 0.8.0 shard marker carries the endpoint id and the render identity the
/// runtime replays back to the server.
...
/// Strips the runtime's own `<!--::topcoat::...-->` bookkeeping comments. They
/// are transport metadata — a tracked-read dependency marker, for instance —
/// not markup, and only the native side has any reason to emit them.
...
async fn the_shard_and_the_htmx_route_render_the_same_fragment() {
    ...
    // The shard's reply also carries a tracked-read dependency marker, which is
    // runtime bookkeeping rather than markup, so it is stripped before the
    // comparison.
