...
async fn main() {
    ...
    let url = std::env::var("SLIPWAY_DATABASE_URL")
        .unwrap_or_else(|_| lab10_solution::database::DEFAULT_DATABASE_URL.to_owned());
    let state = lab10_solution::auth::AppState::connect(&url)
        .await
        .expect("connect to the Slipway database");
    topcoat::start(lab10_solution::app::router(state, Some(assets)))
        .await
        .unwrap();
}
