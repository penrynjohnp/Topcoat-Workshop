...
The workshop currently targets Topcoat and Topcoat CLI 0.8.1, Toasty 0.10.0, and the versions recorded in [`COMPATIBILITY.md`](https://github.com/penrynjohnp/Topcoat-Workshop/blob/main/COMPATIBILITY.md). Topcoat is pre-1.0, so keep the repository checkout and its pins together.
...
The script checks for `rustup`, reports the active toolchain, installs or refreshes `topcoat-cli` 0.8.1 with `--locked`, notes whether the `sqlite3` command is available, and builds Lab 01's solution as a smoke test:
...
> [!WARNING]
> Topcoat CLI 0.8.1 has no top-level `topcoat --version` command. Check the installed version with `cargo install --list`, or use `topcoat fmt --version` for the formatter binary. `topcoat fmt` also has no `--check` flag; the repository formats and then checks for a Git diff.
...
The container provides Rust, Node.js LTS, Azure CLI, `azd`, and Docker-in-Docker. Its post-create command installs Chromium for Marp slide export, `sqlite3`, Topcoat CLI 0.8.1, mdBook 0.5.4, mdbook-mermaid 0.17.1, and mdbook-linkcheck2 0.13.0:
