...
async fn get(path: &str) -> (StatusCode, String, String) {
    let app_state = slipway_capstone::auth::AppState::test().await.unwrap();
    let router = slipway_capstone::app::router(app_state.clone(), None);
...
#[tokio::test]
async fn vendored_card_style_reaches_every_berth() {
    let (status, _, html) = get("/berths").await;
    assert_eq!(status, StatusCode::OK, "{html}");
    assert_eq!(html.matches("data-ui=\"marina-card\"").count(), 4, "{html}");
    assert_eq!(html.matches("before:from-cyan-500").count(), 4, "{html}");
}

#[tokio::test]
async fn shell_uses_inline_icons_and_no_longer_references_the_htmx_cdn() {
    let (status, _, html) = get("/vessels/htmx").await;
    assert_eq!(status, StatusCode::OK);
    assert!(html.contains("<svg"), "{html}");
    assert!(!html.contains("cdn.jsdelivr.net/npm/htmx"), "{html}");
}
