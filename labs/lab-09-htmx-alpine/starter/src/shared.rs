...
/// The markup. The `#[shard]` and the htmx `#[route]` should both render *this*
/// so their fragments stay identical.
#[component]
pub(crate) async fn vessel_matches(query: &str) -> Result<impl View> {
    ...
    Ok(view! {
        <section data-component="vessel-results" data-query=(query)>
            // TODO(lab-09): move the results markup out of the `#[shard]` in
            // `app/_marketing/vessels.rs` and into this component, then have
            // the shard call it. Both transports must render the same HTML.
            <p>"Results go here."</p>
        </section>
    })
}
