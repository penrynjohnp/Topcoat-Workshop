...
- Every claim about how Topcoat behaves is checked against the v0.8.1 guides under `crates/*/docs/` in tokio-rs/topcoat (at the tag, never `main`). Where a page explains internals (`$(...)` cross-compilation, shards, `live!`), say which guide each section draws on.
...
- The CLI at 0.8.1 has no top-level `--version` and `topcoat fmt` has no `--check`; CI formats then `git diff --exit-code`.
