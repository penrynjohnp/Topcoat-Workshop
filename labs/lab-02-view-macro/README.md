...
the formatter that understands them, so run both. Note that CLI 0.8.1 has no `--check` flag, which
...
- **`topcoat: command not found`** — `cargo install topcoat-cli --version 0.8.1 --locked`.
