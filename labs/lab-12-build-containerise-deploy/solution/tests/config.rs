...
use slipway::config::{AppConfig, DatabaseConfig};
...
async fn production_state_marks_session_cookies_secure() {
    ...
    .unwrap();
    let db = slipway::database::test_database().await.unwrap();
    let state = slipway::auth::AppState::from_configured_db(db, config.cookie_key, true);
