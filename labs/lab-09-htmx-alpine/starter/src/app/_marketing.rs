...
use crate::{
    ...
    shared::{self, WorkOrder, berth_card, work_order_exists},
};
use topcoat::{
    Result,
    ...
    Ok(view! {
        ...
        <html lang="en">
            <head>
                ...
                // ANCHOR: htmx-script
                // TODO(lab-09): load htmx here. Pin the version — an htmx major
                // bump changes attribute semantics — with
                // <script src="https://cdn.jsdelivr.net/npm/htmx.org@2.0.10/dist/htmx.min.js" defer="defer"></script>
    ...
    Ok(view! {
        ...
        >
            ...
            <button
                ...
                @click=$(async |_event| {
                    let saved = mark_work_order_complete(id.to_owned()).await;
                    completed.set(saved);
                })
            >
